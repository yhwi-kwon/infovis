// 데이터 로드 및 전처리
d3.csv(
  "https://raw.githubusercontent.com/yhwi-kwon/infovis/main/data_sample.csv"
).then((data) => {
  data.forEach((d) => {
    d["출산율"] = +d["출산율"];
    d["행정기관코드"] = +d["행정기관코드"];
  ...
  const bottom10 = data.sort((a, b) => a["출산율"] - b["출산율"]).slice(0, 10);
  ...
  function updateChart() {
    const filterValue = d3.select("input[name='filter']:checked").node().value;
    ...
    svg
      ...
      .on("mouseover", function (event, d) {
        tooltip
          .html(`<strong>${d["읍면동명"]}</strong><br>출산율: ${d["출산율"]}`)
  ...
  function drawPieChart(code) {
    ...
    svg
      ...
      .append("text")
      .text(
        (d) => `${d.data.age} (${((d.data.value / total) * 100).toFixed(2)}%)`
      )
  ...
  function drawPyramidChart(code) {
    ...
    const y = d3.scaleBand().domain(ageGroups).range([height, 0]).padding(0.1);
